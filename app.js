...
async function openAccountPortal(mode='signin'){
  ...
  try{await window.firefly.openAccountPortal({endpoint,mode});toast('Continue in your browser',mode==='passkeys'?'Add or remove passkeys, then return to Ignifire.':'Choose email, phone, password, one-time code, or passkey.')}
...
function settingsPanelMarkup(tab){
  ...
  return `<header><span class="eyebrow">PRIVACY & CONTROL</span><h2>Decide what can go online</h2><p>Your library and listening history remain local unless you explicitly enable account sync. These switches control optional lookups.</p></header>
    ...
    <div class="settings-section"><h3>Local data</h3><div class="setting-row"><div><b>Analytics and telemetry</b><small>Ignifire does not send listening analytics or usage telemetry</small></div><span class="privacy-status"><i></i>Always off</span></div><div class="setting-row"><div><b>Saved data location</b><small>${esc(dataDirectory||'Browser local storage')}</small></div><button class="ghost" id="openDataFolder" ${dataDirectory?'':'disabled'}>Open folder</button></div><div class="setting-row"><div><b>Reset preferences</b><small>Restore interface, playback, and library defaults without deleting music</small></div><button class="ghost danger" id="resetSettings">Reset settings</button></div></div>`;
}
...
function renderSuno() {
  ...
  if(!sunoConnected){$('#sunoContent').innerHTML=`<div class="empty-state">${icon('suno')}<h2>Connect ApiPass to begin</h2><p>Paste an ApiPass API key. Ignifire encrypts it with Windows and sends it only from the protected main process.</p><button class="primary" id="emptySunoConnect">Connect ApiPass</button></div>`;$('#emptySunoConnect').onclick=connectSunoModal;return}
