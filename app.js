...
const defaultSettings = {
  ...
  gapless: true, crossfade: 4, volume: 72, playbackRate:1, preservePitch:true, normalizeTrackVolume:false, autoplayNext:true, preloadAudio:true, stopAfterCurrent:false,
  ...
  cloudSyncEnabled:false, autoDownloadCloudLibrary:false, autoDeleteLocalAfterCloudUpload:false
};
...
let playbackDryGain = null;
let playbackNormalizationInput = null;
let playbackCompressor = null;
let playbackNormalizedGain = null;
const ACCOUNT_STORAGE_LIMIT_BYTES = 256 * 1024 * 1024 * 1024;
let accountState = { signedIn:false, configured:false, status:'idle', storageUsed:0, storageLimit:ACCOUNT_STORAGE_LIMIT_BYTES, testingUpdatesAuthorized:false };
...
async function initializePersistence() {
  ...
  initializeAccount().finally(initializeUpdater);
}
...
async function initializeAccount(){
  ...
  enforceTestingUpdateAccess();
  updateAccountControl();
  if(currentView==='settings'&&['account','updates'].includes(settingsTab))renderSettings();
}
...
function testingUpdatesAuthorized(){return Boolean(accountState.signedIn&&accountState.testingUpdatesAuthorized)}
function enforceTestingUpdateAccess(){if(settings.updateChannel!=='testing'||accountState.status!=='ready'||testingUpdatesAuthorized())return true;settings.updateChannel='stable';updateState={status:'idle',channel:'stable',available:false,progress:null};if(persistenceReady)saveLibrary({refresh:'none'});return false}
function updateChannelLabel(channel=settings.updateChannel){return channel==='testing'?'Testing · authorized':channel==='beta'?'Beta · beta':'Stable · main'}
...
function initializeUpdater(){
  updateState.channel=settings.updateChannel==='testing'?'testing':settings.updateChannel==='beta'?'beta':'stable';renderUpdateWidget();
...
function settingsPanelMarkup(tab){
  ...
  if(tab==='updates'){const testingOption=testingUpdatesAuthorized()?`<option value="testing" ${settings.updateChannel==='testing'?'selected':''}>Testing · authorized</option>`:'';return `<header><span class="eyebrow">UPDATES</span><h2>Keep Ignifire current</h2><p>Choose a release channel and manage background updates.</p></header><div class="settings-section"><div class="setting-row update-setting-row"><div><b>Update channel</b><small>Stable follows main; Beta follows beta${testingUpdatesAuthorized()?'; Testing follows authorized development builds':''}</small></div><span class="update-setting-controls"><select id="updateChannel"><option value="stable" ${!['beta','testing'].includes(settings.updateChannel)?'selected':''}>Stable · main</option><option value="beta" ${settings.updateChannel==='beta'?'selected':''}>Beta · beta</option>${testingOption}</select><button class="ghost" id="checkForUpdates">Check now</button></span></div><div class="setting-row"><div><b>Update status</b><small>${updateStatusText()}</small></div><button class="ghost" id="showUpdateDetails">Details</button></div></div>`}
...
function renderSettings() {
  ...
  $$('[data-setting-toggle]',view).forEach(button=>button.onclick=()=>{const key=button.dataset.settingToggle;settings[key]=!settings[key];button.classList.toggle('on',Boolean(settings[key]));button.setAttribute('aria-pressed',String(Boolean(settings[key])));if(key==='shuffle')setShuffleEnabled(settings[key],{persist:false});if(key==='repeatMode')repeatMode=settings.repeatMode;applySettings();saveLibrary({refresh:'none'});if(key.startsWith('discord'))configureDiscordPresence();if(key==='normalizeTrackVolume'){ensurePlaybackAudioGraph().catch(()=>{});toast(settings[key]?'Volume normalization on':'Volume normalization off',settings[key]?'Track loudness will be balanced automatically.':'Tracks will play at their original relative loudness.')}if(key==='cloudSyncEnabled'){const syncNow=$('#accountSyncNow');if(syncNow)syncNow.disabled=!settings[key]}if(key==='autoDownloadCloudLibrary'&&settings[key])setCloudDownload(allTracks(),true,{pin:false})});
  ...
  if($('#updateChannel'))$('#updateChannel').onchange=e=>{const requested=e.target.value;settings.updateChannel=requested==='testing'&&testingUpdatesAuthorized()?'testing':requested==='beta'?'beta':'stable';updateState={status:'idle',channel:settings.updateChannel,available:false,progress:null};saveLibrary({refresh:'none'});renderUpdateWidget();checkForUpdates(true)};
...
function importAudioEntries(entries,targetTrack=null){
  ...
  audioEntries.forEach((entry,index)=>{
    ...
    if(targetTrack&&index===0){
      ...
      targetTrack.lyrics=meta.lyrics||targetTrack.lyrics||null;targetTrack.durationSeconds=Number(meta.duration)||targetTrack.durationSeconds||0;const replayGainDb=meta.replayGainDb==null?NaN:Number(meta.replayGainDb);targetTrack.replayGainDb=Number.isFinite(replayGainDb)?replayGainDb:null;
...
async function tryPlayCurrentAudio(){
  try{await ensurePlaybackAudioGraph()}catch(error){console.warn('Volume normalization is unavailable; using direct playback.',error)}
  try{await audio.play()}catch(error){setPlaying(false);markTrackFileError(currentTrack,error)}
}
...
function replayGainForTrack(track=currentTrack){if(track?.replayGainDb==null||track.replayGainDb==='')return null;const value=Number(track.replayGainDb);return Number.isFinite(value)?Math.max(-12,Math.min(12,value)):null}
function configureVolumeNormalization(track=currentTrack){
  if(!playbackAudioContext||!playbackDryGain||!playbackNormalizationInput||!playbackNormalizedGain)return;
  const enabled=Boolean(settings.normalizeTrackVolume),now=playbackAudioContext.currentTime,replayGain=replayGainForTrack(track),inputGain=replayGain==null?1:Math.pow(10,replayGain/20),makeupGain=replayGain==null?1.38:1;
  playbackNormalizationInput.gain.setTargetAtTime(inputGain,now,.025);
  playbackDryGain.gain.setTargetAtTime(enabled?0:1,now,.018);
  playbackNormalizedGain.gain.setTargetAtTime(enabled?makeupGain:0,now,.018);
}
async function ensurePlaybackAudioGraph(){
  ...
  if(!playbackAudioContext){
    playbackAudioContext=new AudioContextClass();playbackAudioSource=playbackAudioContext.createMediaElementSource(audio);playbackDryGain=playbackAudioContext.createGain();playbackNormalizationInput=playbackAudioContext.createGain();playbackCompressor=playbackAudioContext.createDynamicsCompressor();playbackNormalizedGain=playbackAudioContext.createGain();
    playbackCompressor.threshold.value=-26;playbackCompressor.knee.value=24;playbackCompressor.ratio.value=6;playbackCompressor.attack.value=.008;playbackCompressor.release.value=.3;
    playbackAudioSource.connect(playbackDryGain).connect(playbackAudioContext.destination);playbackAudioSource.connect(playbackNormalizationInput).connect(playbackCompressor).connect(playbackNormalizedGain).connect(playbackAudioContext.destination);
    playbackDryGain.gain.value=1;playbackNormalizedGain.gain.value=0;
  }
  configureVolumeNormalization();if(playbackAudioContext.state==='suspended')await playbackAudioContext.resume();return true;
}
