...
function updateForm() {
  ...
  $('#formTitle').textContent = resettingPassword ? 'Set a secure password' : mode === 'signup' ? 'Create your Firefly account' : 'Sign in to Firefly';
  ...
  $('#switchCopy').textContent = mode === 'signup' ? 'Already have an account?' : 'New to Firefly?';
...
async function refreshSession() {
  ...
  if (user) { $('#userName').textContent=user.name||'Firefly listener';$('#userIdentity').textContent=user.phoneNumber||(!String(user.email||'').endsWith('@phone.firefly.invalid')?user.email:'')||'Secure account'; }
  return user;
}
...
$('#authForm').onsubmit=async event=>{event.preventDefault();showMessage('');const identifier=$('#identifier').value.trim(),password=$('#password').value,otp=$('#otp').value.trim(),name=$('#displayName').value.trim()||'Firefly listener';try{
...
$('#addPasskey').onclick=async()=>{try{unwrap(await auth.passkey.addPasskey({name:`Firefly passkey · ${new Date().toLocaleDateString()}`}));alert('Passkey added successfully.')}catch(error){alert(error.message)}};
